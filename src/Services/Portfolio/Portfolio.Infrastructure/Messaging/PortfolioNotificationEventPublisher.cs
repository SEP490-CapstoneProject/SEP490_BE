...
public class PortfolioNotificationEventPublisher : IPortfolioNotificationEventPublisher
{
    ...
    public async Task PublishReportCreatedAsync(PortfolioReportCreatedNotificationEvent evt, CancellationToken cancellationToken = default)
    {
        var uri = _configuration["RabbitMQ:Uri"];
        var factory = new ConnectionFactory();
        ...
        else
        {
            ...
            _logger.LogInformation(
                "Publishing portfolio report notification with RabbitMQ host {Host}, port {Port}, vhost {VHost}",
                host, factory.Port, virtualHost);
        }
        ...
        const string exchange = "skillsnap.events";
        const string routingKey = "portfolio.reported";
        ...
        _logger.LogInformation(
            "Published portfolio report notification. PortfolioId={PortfolioId}, ReporterUserId={ReporterUserId}",
            evt.PortfolioId, evt.ReporterUserId);
    }
