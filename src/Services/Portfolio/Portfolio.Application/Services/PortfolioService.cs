...
public class PortfolioService : IPortfolioService
{
        ...
        Dictionary<string, IFormFile> fileMap)
    {
        ...
        foreach (var blockRequest in request.Blocks.OrderBy(b => b.Order))
        {
            ...
            await handler.HandleAsync(portfolio, block, blockRequest.Data, fileMap);
        ...
        _logger.LogInformation("Portfolio {Id} created with {BlockCount} blocks for user {UserId}",
            portfolio.Id, portfolio.Blocks.Count, request.UserId);

        return new CreatePortfolioResponse { PortfolioId = portfolio.Id };
    }
